...
#include "Config.h"
#include "Input/RemoteInputSource.h"
...
RemoteInputSource::RemoteInputSource() = default;

RemoteInputSource::~RemoteInputSource()
{
	pxAssert(m_controllers.empty());
}

bool RemoteInputSource::Initialize(SettingsInterface& si, std::unique_lock<std::mutex>& settings_lock)
{
	LoadSettings(si);
...
void RemoteInputSource::UpdateSettings(SettingsInterface& si, std::unique_lock<std::mutex>& settings_lock)
{
...
bool RemoteInputSource::ReloadDevices()
{
...
void RemoteInputSource::Shutdown()
{
	ShutdownSubsystem();
}

void RemoteInputSource::LoadSettings(SettingsInterface& si)
{
...
u32 RemoteInputSource::GetRGBForPlayerId(SettingsInterface& si, u32 player_id)
{
...
u32 RemoteInputSource::ParseRGBForPlayerId(const std::string_view& str, u32 player_id)
{
...
void RemoteInputSource::SetHints()
{
	if (const std::string upath = Path::Combine(EmuFolders::DataRoot, CONTROLLER_DB_FILENAME); FileSystem::FileExists(upath.c_str()))
	{
		Console.WriteLn(Color_StrongGreen, fmt::format("RemoteInputSource: Using Controller DB from user directory: '{}'", upath));
	...
	{
		Console.WriteLn(Color_StrongGreen, "RemoteInputSource: Using Controller DB from resources.");
		SDL_SetHint(SDL_HINT_GAMECONTROLLERCONFIG_FILE, rpath.c_str());
	}
	else
	{
		Console.Error(fmt::format("RemoteInputSource: Controller DB not found, it should be named '{}'", CONTROLLER_DB_FILENAME));
	}
...
std::vector<std::pair<std::string, std::string>> RemoteInputSource::EnumerateDevices()
{
...
std::optional<InputBindingKey> RemoteInputSource::ParseKeyString(const std::string_view& device, const std::string_view& binding)
{
...
TinyString RemoteInputSource::ConvertKeyToString(InputBindingKey key)
{
	TinyString ret;
...
TinyString RemoteInputSource::ConvertKeyToIcon(InputBindingKey key)
{
	TinyString ret;
...
bool RemoteInputSource::ProcessSDLEvent(const SDL_Event* event)
{
	switch (event->type)
	{
		case SDL_CONTROLLERDEVICEADDED:
		{
			Console.WriteLn("(RemoteInputSource) Controller %d inserted", event->cdevice.which);
		...
		{
			Console.WriteLn("(RemoteInputSource) Controller %d removed", event->cdevice.which);
		...
		case SDL_JOYDEVICEADDED:
		{
			...
			Console.WriteLn("(RemoteInputSource) Joystick %d inserted", event->jdevice.which);
		...
		case SDL_JOYDEVICEREMOVED:
		{
			...
			Console.WriteLn("(RemoteInputSource) Joystick %d removed", event->jdevice.which);
			CloseDevice(event->cdevice.which);
			return true;
		}
...
SDL_Joystick* RemoteInputSource::GetJoystickForDevice(const std::string_view& device)
{
...
RemoteInputSource::ControllerDataVector::iterator RemoteInputSource::GetControllerDataForJoystickId(int id)
{
	return std::find_if(m_controllers.begin(), m_controllers.end(), [id](const ControllerData& cd) { return cd.joystick_id == id; });
}

RemoteInputSource::ControllerDataVector::iterator RemoteInputSource::GetControllerDataForPlayerId(int id)
{
	return std::find_if(m_controllers.begin(), m_controllers.end(), [id](const ControllerData& cd) { return cd.player_id == id; });
}

int RemoteInputSource::GetFreePlayerId() const
{
...
bool RemoteInputSource::OpenDevice(int index, bool is_gamecontroller)
{
	...
	{
		Console.Error("(RemoteInputSource) Failed to open controller %d", index);
		return false;
	}
	...
	for (auto it = m_controllers.begin(); it != m_controllers.end(); ++it)
	{
		...
		{
			Console.Error("(RemoteInputSource) Controller %d, instance %d, player %d already connected, ignoring.", index, joystick_id, player_id);
			if (gcontroller)
	...
	if (player_id < 0 || GetControllerDataForPlayerId(player_id) != m_controllers.end())
	{
		...
		Console.Warning("(RemoteInputSource) Controller %d (joystick %d) returned player ID %d, which is invalid or in "
	...
	Console.WriteLn("(RemoteInputSource) Opened %s %d (instance id %d, player id %d): %s", is_gamecontroller ? "game controller" : "joystick",
	...
	if (gcontroller)
	{
		...
		Console.WriteLn("(RemoteInputSource) Controller %d has %d axes and %d buttons", player_id, num_axes, num_buttons);
	}
	else
	{
		...
		Console.WriteLn("(RemoteInputSource) Joystick %d has %d axes, %d buttons and %d hats", player_id,
	...
	{
		Console.WriteLn("(RemoteInputSource) Rumble is supported on '%s' via gamecontroller", name);
	}
	else
	{
		...
		if (haptic)
		{
			...
			{
				Console.Error("(RemoteInputSource) Failed to create haptic left/right effect: %s", SDL_GetError());
				...
				{
					Console.Error("(RemoteInputSource) No haptic rumble supported: %s", SDL_GetError());
		...
		if (cd.haptic)
			Console.WriteLn("(RemoteInputSource) Rumble is supported on '%s' via haptic", name);
	}

	if (!cd.haptic && !cd.use_game_controller_rumble)
		Console.Warning("(RemoteInputSource) Rumble is not supported on '%s'", name);
...
bool RemoteInputSource::CloseDevice(int joystick_index)
{
...
bool RemoteInputSource::HandleControllerAxisEvent(const SDL_ControllerAxisEvent* ev)
{
...
bool RemoteInputSource::HandleControllerButtonEvent(const SDL_ControllerButtonEvent* ev)
{
	auto it = GetControllerDataForJoystickId(ev->which);
	if (it == m_controllers.end())
		return false;
...
bool RemoteInputSource::HandleJoystickAxisEvent(const SDL_JoyAxisEvent* ev)
{
	auto it = GetControllerDataForJoystickId(ev->which);
	if (it == m_controllers.end())
		return false;
...
bool RemoteInputSource::HandleJoystickButtonEvent(const SDL_JoyButtonEvent* ev)
{
	auto it = GetControllerDataForJoystickId(ev->which);
	if (it == m_controllers.end())
		return false;
...
bool RemoteInputSource::HandleJoystickHatEvent(const SDL_JoyHatEvent* ev)
{
	auto it = GetControllerDataForJoystickId(ev->which);
...
std::vector<InputBindingKey> RemoteInputSource::EnumerateMotors()
{
...
bool RemoteInputSource::GetGenericBindingMapping(const std::string_view& device, InputManager::GenericInputBindingMapping* mapping)
{
	if (!device.starts_with("SDL-"))
		return false;
...
void RemoteInputSource::UpdateMotorState(InputBindingKey key, float intensity)
{
...
void RemoteInputSource::UpdateMotorState(InputBindingKey large_key, InputBindingKey small_key, float large_intensity, float small_intensity)
{
...
void RemoteInputSource::SendRumbleUpdate(ControllerData* cd)
{
